...
[![website: newsletter builder](https://img.shields.io/badge/website-newsletter%20builder-003F88.svg)](https://basilechretien.github.io/Newsletter-graduate-school-medicine/)
...
[![tests: 694 passing](https://img.shields.io/badge/tests-694%20passing-brightgreen.svg)](tests/)
...
- **macOS:** Macs ship with Python preinstalled, so you usually have nothing to do here. If the launcher later says Python is missing or too old, come back and install from <https://www.python.org/downloads/> the same way as Windows.
...
- **Stack:** Python 3.11+ (CI runs 3.12 and 3.14), `python-docx`, `Jinja2`, `css_inline`, `click`, `pytest`, `pywin32` (Windows only).
...
- **Keeping dependencies current:** Dependabot (`.github/dependabot.yml`) opens weekly pull requests for `requirements*.txt` and the workflow actions. What it cannot see — the Pyodide runtime (and whether a newer one still ships `css-inline`), the browser build's package versions against `requirements.txt`, the wheel vendored from PyPI, and Python's end-of-life dates — is checked every Monday by `.github/workflows/update-check.yml`, which keeps one issue labelled `dependencies` current. Run the same check locally with `python tools/check_updates.py`.
- **README test counts:** CI fails when the "tests" badge or the "Under the hood" figures disagree with the suite. Rewrite them with `python tools/readme_figures.py --fix`.
- **The website engine:** `.github/workflows/web-engine.yml` runs the real browser build under Node -- the vendored Pyodide runtime, the packages `web/app.js` loads, the committed bundle -- and checks it produces the same email as the desktop build, on every relevant change and weekly. The runtime files are also mirrored as a GitHub pre-release (`pyodide-runtime-<version>`) that `web/vendor_pyodide.py` falls back to, so deploys keep working if the CDN drops them. Details in [`web/README.md`](web/README.md).

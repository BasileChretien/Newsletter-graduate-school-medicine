 ...
 *
 * Loads the vendored Pyodide runtime and the packages the page loads, both
 * from the paths web/app.js names (so neither can drift from the page), and
 * the committed web/meridian-bundle.zip, then calls
...
// The directory the page boots from: `./pyodide/<version>/`.
const indexUrl = appJs.match(/const PYODIDE_INDEX_URL = "([^"]+)";/);
if (!indexUrl) throw new Error("PYODIDE_INDEX_URL not found in web/app.js");
const pyodideDir = path.resolve(webDir, indexUrl[1]);   // no trailing separator
...
// A bare name comes from the runtime's lockfile; a wheel loaded by path is
...
function pagePackages() {
  ...
  return entries.map(([, entry]) => (entry.startsWith("./")
    ? posix(path.join(webDir, entry))
    : entry));
}

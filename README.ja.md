...
- **Pythonが未インストール:** インストールしてから再実行してください (セットアップ手順2)。
- **macOSのみ:** ターミナルでフォルダに移動し、`chmod +x "Make Newsletter.command"` を一度実行してください。

**❓ 「ERROR: file not found」と表示される**
入力したWordファイルがプロジェクトフォルダ内にありません。`issue-3.docx` を `Make Newsletter.bat` と同じフォルダに移動してください。

**❓ どのメールアプリが使われますか?**
お使いのコンピュータで `mailto:` リンクのデフォルトに設定されているアプリです。
- Windows: 設定 → アプリ → 既定のアプリ → メール
- macOS: メールアプリ → 環境設定 → 一般 → デフォルトメールアプリ

**❓ もう一度作り直したい**
ランチャーをもう一度ダブルクリックして同じ号数を入力してください。前回の出力は上書きされます。
